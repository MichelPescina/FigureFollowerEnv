...
class FigFollower(gym.Env):
    ...
    Reward is given as the robot approximates the target signal (1 reward per meter),
    to be precise the amount of reward is proportial to the improvement over the previous
    distance to the target signal (how many meters is closer compared to the last time evaluated).
    There is not a reward limit as the maps can contain different number of signals.
    A penalization is given each time the robot collides with something.
    """
    ...
    def _compute_reward(self):
        ...
        return improvement - collisions * 10
